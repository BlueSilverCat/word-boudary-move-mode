...
class Controller {
  constructor({ subwordBoundary = true, autoSelect = false } = {}) {
    ...
    this.keyBindings = new KeyBindings({ "source": "wbmm", "keyBindings": wbmmKeyBindings, "priority": 0 });
  }
  ...
  resetSelection() {
    if (this.editor.getSelectedBufferRange().isEmpty() === false) {
      this.editor.setSelectedBufferRange([this.cursor.getBufferPosition(), this.cursor.getBufferPosition()]); // this.editor.getLastSelection().clear();
      return true;
    }
    return false;
  }
  ...
  getFuncSetKeyBindings() {
    return (keyBindings) => {
      this.keyBindings.setKeyBindings(keyBindings, true);
    };
  }
}
